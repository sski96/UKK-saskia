...
<div class="mt-3 card">
    ...
    <div class="card-body">
        ...
        <p class="card-text">
            ...
            Tanggal Check In : {{ \Carbon\Carbon::parse($reservation->check_in)->setTimezone('Asia/Jakarta')->formatLocalized('%A, %d %B %Y Pukul : %H.%M ') }} <br>
            Tanggal Check Out : {{ \Carbon\Carbon::parse($reservation->check_out)->setTimezone('Asia/Jakarta')->formatLocalized('%A, %d %B %Y Pukul : %H.%M ') }} <br>            
            Status : 
            ...
            <!-- Tombol Kembali -->
            <a href="/dashboard" class="mt-3 btn btn-sm btn-secondary">Kembali</a>
        </p>
    </div>
</div>      
